...
- 最終選択した地域をローカルストレージに保存し、次回自動で復元
...
### ローカルでの動かし方
- 簡易サーバー例: `python3 -m http.server 8000` をリポジトリ直下で実行し、`http://localhost:8000/index.html` を開く。
- 直接ファイルを開く場合でも動作しますが、一部ブラウザでローカルファイルのfetchが制限されるため、サーバー経由が安全です。
